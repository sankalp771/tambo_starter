...
const MessageContent = React.forwardRef<HTMLDivElement, MessageContentProps>(
  (
    ...
    // Explicit color selection based on role
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "relative block rounded-2xl px-4 py-2.5 text-[15px] leading-relaxed transition-all duration-200 font-medium max-w-full",
          textColor,
          className,
        )}
      ...
      >
        {showLoading && !message.reasoning ? (
          ...
          <div
            className={cn("break-words overflow-hidden", !markdown && "whitespace-pre-wrap")}
            data-slot="message-content-text"
          >
            {!hasContent && !isLoading ? (
              <span className="opacity-40 italic text-xs">Empty message</span>
            ) : (
            ...
            {message.isCancelled && (
              <span className="text-muted-foreground text-[10px] ml-2 uppercase font-black">cancelled</span>
            )}
          </div>
        )}
      </div>
